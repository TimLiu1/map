function start(url) {
    RequestGet('/network/list', (err, data) => {
        var cities = data.data;
        var citiesData = getCityData(cities);
        rendMap(cities, citiesData)
        // return citiesData;
    })
}
...
function connectLine(links, linksData, path, circle,cities) {
    let linksFeatures = links.selectAll("path")
        .data(linksData)
    linksFeatures.enter()
        .append("path")
        .attr("class", "arcs")
        ...
        .call(linetransition)
    return linksFeatures;
}

function rendMap(cities, citiesData) {
        ...
        function init() {
            ...
            devFeatures.enter()
                ...
                .on('click', function () {//选择所有的点添加点击事件
                    ...
                    start('/network/list?mainDevice=pvg0-wr03')
                })
                .attr("d", function (d) {
                    return path(circle.clip(d));
                }).attr("id", function (d) {
            ...
            linksFeatures = connectLine(links, linksData, path, circle,cities)
...
start('http://localhost:8008/network/list');
